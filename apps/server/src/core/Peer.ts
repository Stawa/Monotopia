...
const PUNCH_SLOT_MAP: { [key: number]: keyof PeerData["clothing"] } =
...
export class Peer extends OldPeer<PeerData> {
  ...
  public countryState() {
    const country = (pe: Peer) =>
      `${pe.country}|${pe.data.level >= 125 ? NameStyles.MAX_LEVEL : ""}`;

    this.send(
      Variant.from({ netID: this.data.netID }, "OnCountryState", country(this)),
    );
    ...
    if (world) {
      world.every((p) => {
        if (p.data.netID !== this.data.netID) {
          ...
          this.send(
            Variant.from({ netID: p.data.netID }, "OnCountryState", country(p)),
          );
        }
      });
    }
    ...
    for (const heartMonitor of this.data.heartMonitors) {
      ...
      if (world.data.playerCount) {
        ...
        for (const heartMonitorIndex of heartMonitorIndexList) {
          tiles.push(
            tileFrom(this.base, world, world.data.blocks[heartMonitorIndex]),
          );
        }
      ...
      TankPacket.from({
        ...
        data: () => {
          ...
          inventory.items.forEach((item) => {
            ...
            const isEquipped = Object.values(this.data.clothing).includes(
              item.id,
            );
      ...
      world.every((p) => {
        p.send(
          Variant.from(
            ...
            "OnSetClothing",
            [clothing.hair, clothing.shirt, clothing.pants],
            [clothing.feet, clothing.face, clothing.hand],
            [clothing.back, clothing.mask, clothing.necklace],
            this.getSkinColor(),
  ...
  public sendEffect(eff: number, ...args: Variant[]) {
    ...
    if (world) {
      world.every((p) => {
        p.send(
          Variant.from("OnParticleEffect", eff, [
            (this.data.x as number) + 10,
            (this.data.y as number) + 16,
          ]),
          ...args,
        );
      });
        ...
        world.every((p) => {
          p.send(
            Variant.from(
              "OnTalkBubble",
              this.data.netID,
              `${this.data.displayName} is now level ${this.data.level}!`,
            ),
            Variant.from(
              "OnConsoleMessage",
              `${this.data.displayName} is now level ${this.data.level}!`,
  ...
  public sendOnPlayPositioned(file: string, opts?: VariantOptions) {
    this.send(Variant.from(opts, "OnPlayPositioned", file));
  }
  ...
  public sendConsoleMessage(message: string, opts?: VariantOptions) {
    this.send(Variant.from(opts, "OnConsoleMessage", message));
  }
  ...
  public canAddItemToInv(itemID: number, amount: number = 1): boolean {
    ...
    if (
      ...
      (!inventoryItem &&
        this.data.inventory.items.length >= this.data.inventory.max)
    ) {
      return false;
    }
    return true;
  ...
  public setGems(amount: number, skip_animation: number = 0) {
    ...
    ); // Param 2 maybe for supporter status?
  }
}
